//  V.5.2b Dark Mode  17.09.2022 Added a squelch function By Lyle Hancock Sr.
//  Replaced the "CHIP" button with "SQLCH" V.5.2a Dark mode 08.09.2022 Fully
//  RDS services - RDS Station  name - RDS News & info - RDS Time station Mod.by
//  IU4ALH ;
//                     led colors on S-Meter bar -
//  V4.0    23-02-2022 Binns MOD full rebuilding design and functionaly. See
//  owerview on youtube channel: V3.4    24-11-2021 Bug support. V3.4 24-11-2021
//  Memory added for BFO in each band when crystal is in use. V3.3    02-11-2021
//  V3.2.6b 29-10-2021  5351 calibration
//  V3.2.4  03-10-2021 100 Hz & 10 Hz added in SSB. Many changes in control
//  interface. Bug support.
//                    SI5351 added, replacing crystal and also used as BFO. All
//                    steps and bandwidth per modulation are now stored in
//                    memory.
//  V3.2.3  29-09-2021 Added Sprite buttons from Jim Yasuda.
//  This sketch is based on the si4735 Library of Ricardo PU2CLR. Thanks for the
//  very nice work.

//  This sketch uses  a 2.8 inch 240*320 touch-screen with ILI9341, ESP32
//  WROOM-32 and Rotary Encoder. The radio is fully controlled by the
//  (Touch)Screen and Rotary Encoder This sketch uses the Rotary Encoder Class
//  implementation from Ben Buxton (the source code is included together with
//  this sketch). For the touch-screen the library TFT_eSPI is used. The
//  configuration setup-file: setup1_ILI9341 is also included. Also a schematic
//  drawing is available.

//  ABOUT SSB PATCH:
//  First of all, it is important to say that the SSB patch content is not part
//  of this library. The paches used here were made available by Mr. Vadim
//  Afonkin on his Dropbox repository. It is important to note that the author
//  of the SI473x library does not encourage anyone to use the SSB patches
//  content for commercial purposes. In other words, this library only supports
//  SSB patches, the patches themselves are not part of this library. This
//  sketch will download a SSB patch to your SI4735 device (patch_init.h). It
//  will take about 8KB of the Arduino memory. In this context, a patch is a
//  piece of software used to change the behavior of the SI4735 device. There is
//  little information available about patching the SI4735. The following
//  information is the understanding of the author of this project and it is not
//  necessarily correct. A patch is executed internally (run by internal MCU) of
//  the device. Usually, patches are used to fixes bugs or add improvements and
//  new features of the firmware installed in the internal ROM of the device.
//  Patches to the SI4735 are distributed in binary form and have to be
//  transferred to the internal RAM of the device by the host MCU (in this case
//  Arduino). Since the RAM is volatile memory, the patch stored into the device
//  gets lost when you turn off the system. Consequently, the content of the
//  patch has to be transferred again to the device each time after turn on the
//  system or reset the device.

//  ATTENTION: The author of this project does not guarantee that procedures
//  shown here will work in your development environment. Given this, it is at
//  your own risk to continue with the procedures suggested here. This library
//  works with the I2C communication protocol and it is designed to apply a SSB
//  extension PATCH to CI SI4735-D60. Once again, the author disclaims any
//  liability for any damage this procedure may cause to your SI4735 or other
//  devices, like an ATS-25 that you are using. This sketch SHOULD work with the
//  Chinese KIT ATS-25 sold on AliExpress, eBay etc. The author of this sketch
//  and Arduino Library does not know the seller of this kit and does not have a
//  commercial relationship with any commercial product that uses the Arduino
//  Library. It is important you understand that there is no guarantee that this
//  sketch will work correctly in your current product. SO, DO NOT TRY IT IF YOU
//  DON'T KNOW WHAT ARE YOU DOING. YOU MUST BE ABLE TO GO BACK TO THE PREVIOUS
//  VERSION IF THIS SKETCH DOES NOT WORK FOR YOU.

//  Library TFT_eSPI you may download from here :
//  https://github.com/Bodmer/TFT_eSPI Library Rotary is provided with the
//  program Library SI4735 you may download from here   :
...
//  Encoder        1,2,3
//  |------------|------------------|------------|------------|------------|
//  Encoder switch 4,5 |   Vcc      |     3V3     | 01 |    Vcc     | | | pin 33
//  with 18K to 3.3 volt and 18K to ground. |   GND      |     GND     | 02 |
//  GND     |     2,4    |            |        pin 32 (Beeper) via 2K to base V1
//  BC547 |   CS       |     15      | 03 |            |            | |
//  Collector via beeper to 5v |   Reset    |      4      | 04 |            | |
//  |        Emmitor to ground |   D/C      |      2      | 05 |            | |
//  | |   SDI      |     23      | 06 |            |            |            |
//  Encoder        1,2,3 |   SCK      |     18      | 07 |            | | |
//  Encoder switch 4,5 |   LED Coll.|     14 2K   | 08 |            | | |
//  Display LED |   SDO      |             | 09 |            |            | |
//  Emmitor  V2 BC557 to 3.3 V |   T_CLK    |     18      | 10 |            | |
//  |        Base with 2K to pin 14 (PIN_DISPLAY_LED) |   T_CS     |      5 | 11
//  |            |            |            |        Collector to led pin display
//  |   T_DIN    |     23      | 12 |            |            |            |
//  |   T_DO     |     19      | 13 |            |            |            |
//  |   T_IRQ    |     34      | 14 |            |            |            |
//  |            |     12      |    |   Reset    |            |            |
...
void drawButton(uint8_t, uint8_t, uint8_t, arduino::String);
void FreqDispl();
...
void tftRusPrint(arduino::String, int, int);
...
// #define R_ATTENNUATOR \
//   ((197.5f + 99.5f) / 99.5f)  // Voltage divider resistor values
// // #define CONVERSION_FACTOR (1 << AD_RESOLUTION) // This was 4096
// #define ADC_MAX_VALUE \
//   ((1 << AD_RESOLUTION) - 1.0f)  // This will be 4095.0f for 12-bit ADC
// #define VOLTAGE_ATT_CORR \
//   -0.02  // Correction for the actual measured voltage on the divider

//   // ADC érték átalakítása feszültséggé
//   pinMode(A0, INPUT);
//   float voltageOut = (analogRead(A0) * V_REFERENCE) / ADC_MAX_VALUE;
//   Serial << "Vout: " << voltageOut << endl;
//   voltageOut += VOLTAGE_ATT_CORR;  // csalunk egyet, ez a valós feszültség
//                                    // (műszeres méréssel) az osztón
 ...
 */
inline float readBatterry() {
  // ADC érték átalakítása feszültséggé
  float voltageOut =
      (analogRead(PIN_VBUS_DIVIDER) * V_REFERENCE) / CONVERSION_FACTOR;
...
// ====================Display======================
// Rotate 0   // vertical   0 // Calibration code for touchscreen : for 2.8 inch
// // { 387, 3530, 246, 3555, 4 } Rotate 90  // horizontal 1 // Calibration code
// for touchscreen : for 2.8 inch // { 387, 3530, 246, 3555, 7 } Rotate 180 //
// vertical   2 // Calibration code for touchscreen : for 2.8 inch // { 258,
// 3566, 413, 3512, 2 } Rotate 270 // horizontal 3 // Calibration code for
// touchscreen : for 2.8 inch // { 387, 3530, 246, 3555, 1 }
#define SCREEN_V 0
#define SCREEN_H 1
...
int tftRusCursiveLevel =
    4;  // степень наклона курсива 1 - сильный, 2 - средний, 4 - слабый
//-----------------------------END TFT RUS-------------------------------
...
const uint16_t size_content =
    sizeof ssb_patch_content;  // see ssb_patch_content in patch_full.h or
                               // patch_init.h
...
#define MIN_ELAPSED_AudMut_TIME \
  0  // Noise surpression SSB in mSec. 0 mSec = off //Was 0 (LWH)
...
bool SquelchUsesRSSI =
    true;  // When true, the squelch uses RSSI, when false the squelch uses SNR
...
int posScanLast;
float SCANstep;
bool SCANpause = true;  // LWH - SCANpause must be initialized to a value else
                        // the squelch function will not work correctly.
...
const uint8_t RetroStationPos[] = {43,  55,  67,  79,  91,  103,
...
const char *Keypathtext[] = {"1", "2", "3", "4",  "5",   "6",   "7", "8",
                             "9", ".", "0", "OK", "DEL", "CLS", "X"};
...
Band band[] = {
    ...
    {"800M", LW_BAND_TYPE, AM, 280, 470, 284, 1, 0,
     0},  // Ham  800M    3 // LWH - was "BACON"
...
DigNum dn[] = {{0, Xdignum, 21, Ydignum, 35, 0},
...
RotaryEncoder rotaryEncoder =
    RotaryEncoder(PIN_ENCODER_CLK, PIN_ENCODER_DT, PIN_ENCODER_SW);
...
void scanOPTpack() {
  //=======================================================================================
  SCANscale =
      uint8_t((autoSCANstep * 128) + (maxSCANstep * 8) + (minSCANstep * 8));
...
void setup() {
  ...
  // Calibration example with 10 MHz replace 26613UL with your figure.
  ...
  if (EEPROM.read(offsetEEPROM) != storage.chkDigit or
      digitalRead(PIN_ENCODER_SW) == LOW) {
    beeper.error();
  ...
  ITimer1.attachInterruptInterval(TIMER1_INTERVAL_USEC,
                                  hardwareTimerHandler1_ForRotary);
  ...
  if (bandIdx == 0)
    si4735.setup(PIN_SI4753_RESET, -1, POWER_UP_FM, SI473X_ANALOG_AUDIO,
                 XOSCEN_RCLK);  // Start in FM
  else
    si4735.setup(PIN_SI4753_RESET, -1, POWER_UP_AM, SI473X_ANALOG_AUDIO,
                 XOSCEN_RCLK);  // Start in AM
  if (bandIdx != 0) si4735.setAM();
#endif

  if (!displayPower) {
    /////////////////////////////////ledcWrite(LedChannelforTFT,
    /// currentBrightness);
  }
  freqstep = 1000;  // hz
  previousBFO = -1;
  ...
  //////////////////////////////////////////////    xTaskCreate(SaveInEeprom,
  ///"SaveInEeprom", 2048, NULL, 1, NULL);
  delay(10);
...
void useBand() {
  ...
  if ((band[bandIdx].bandType == MW_BAND_TYPE) ||
      (band[bandIdx].bandType == LW_BAND_TYPE)) {
...
void loadSSB() {
  ...
  // delay(50);
  // Parameters
  // AUDIOBW - SSB Audio bandwidth; 0 = 1.2KHz (default); 1=2.2KHz; 2=3KHz;
  // 3=4KHz; 4=500Hz; 5=1KHz; SBCUTFLT SSB - side band cutoff filter for band
  // passand low pass filter ( 0 or 1) AVC_DIVIDER  - set 0 for SSB mode; set 3
  // for SYNC mode. AVCEN - SSB Automatic Volume Control (AVC) enable;
  // 0=disable; 1=enable (default). SMUTESEL - SSB Soft-mute Based on RSSI or
  // SNR (0 or 1). DSP_AFCDIS - DSP AFC Disable or enable; 0=SYNC MODE, AFC
  // enable; 1=SSB MODE, AFC disable.
...
void Freqcalq(int keyval) {
  ...
  if (keyval > 11) {
    ...
    if (Decipoint) {
      ...
      if ((DisplayfreqNew + keyval) <= 30000)
        DisplayfreqNew = (DisplayfreqNew + keyval) * 10;
    }
  }
  tft.setTextDatum(BL_DATUM);
  tft.setTextSize(3);
  float realFreq = (DisplayfreqNew / 10) + dpfrq;
  if ((realFreq > 0 and realFreq < 31) or
      (realFreq >= 64 and realFreq <= 108) or
      (realFreq > 152 and realFreq <= 30000))
  ...
  if (DisplayfreqNew > 0) {
    ...
    tft.print("0");
  if (!Decipoint and DisplayfreqNew >= (band[0].minimumFreq / 10) and
      DisplayfreqNew <= (band[0].maximumFreq / 10))
...
void Smeter() {
  ...
  while (met > 15 and tik < 15) {
    tft.fillRect(Xsmtr + 20 + ((tik - 9) * 16) + 108, Ysmtr + 38, 14, 6,
                 TFT_GREEN);
    met -= 16;
    tik++;
  }
  if (tik == 15 and met > 4) {
    ...
    tft.fillRect(Xsmtr + 22 + spoint - met, Ysmtr + 38,
                 207 - (2 + spoint) + met, 6, TFT_BLACK);
  }
}
...
void brightnessIndicator(int val) {
  ...
  tft.fillRect(XVolInd + 17 + val, YVolInd + 16, 130 - (2 + val), 6,
               TFT_MAROON);
}
...
void saver() {
  ...
  if (saverOn) {
    ...
    if (SCANbut)
      freq = currentScanFreq +
             int((currentScanLine - 159 + deltaScanLine) * SCANstep);
    else if (currentMode == LSB || currentMode == USB || currentMode == CW)
      freq -= int(currentBFO / 1000);
  }

  // WAIT activity
  while (((tftPressed == false) and
          (rotaryEncoderResult.direction == RotaryEncoder::Direction::NONE) and
          (rotaryEncoderResult.buttonState == RotaryEncoder::Open) and
...
void loop() {
  ...
  if (((FirstLayer == true) or (ThirdLayer == true)) and (bright == false) and
      (squelch == false))
  ...
  while (((tftPressed == false) and
    ...
    tftPressed = tft.getTouch(&x, &y);
    if ((elapsedSaver + (saverTime * 60000)) < millis() and
        (saverOn or displayOff))
      saver();
    showtimeRSSI();
    if (batShow) Battery();
    ...
    if (RETRObut) {
      if (tftRusLength(String(presetNameLoad())) > (10 + (screenV * 8)) and
          (elapsedScroll + 200) < millis()) {
        textScroll++;
        if (textScroll == tftRusLength(String(presetNameLoad())) + 5)
          textScroll = 0;
        tftRusSetFont(T1516_T);
        tftRusSetSize(1);
    ...
    if (scrollRetro) {
      ...
      while (i <= lastPreset and scrollRetro) {
        ...
        if ((scrollRetro == 1 and freq > currentRetroFreq and
             freq <= (currentRetroFreq + band[bandIdx].currentStep)) or
            (scrollRetro == -1 and freq < currentRetroFreq and
             freq >= (currentRetroFreq - band[bandIdx].currentStep))) {
      ...
      if (scrollRetro) {
        ...
        if ((scrollRetro == 1 and
             (currentRetroFreq + band[bandIdx].currentStep) <= tmpmax) or
            (scrollRetro != 1 and
             (currentRetroFreq - band[bandIdx].currentStep) >= tmpmin)) {
          if (scrollRetro == 1)
            si4735.frequencyUp();
          else
    ...
    if (PRESbut) {
      if (directScroll and (elapsedScroll + 200) < millis()) {
        ...
        tftRusSetCut(0, 0);
        if (textScroll ==
            (tftRusLength(String(preset[currentPRES].PresetName)) - 18)) {
          elapsedScroll = millis() + 3000;
          directScroll = -1;
        } else if (!textScroll) {
          elapsedScroll = millis() + 3000;
    ...
    if (MEMOadd) {
      if (posMemoName == 20) {
        ...
        for (int i = 0; i < 21; i++)
          MemoBank[currentMemo].Name[i] = addMemoName[i];
        drawButtons(L_MEMO);
        displMEMO();
      } else {
        int d = !screenV * 40;
        ...
        if (posMemoName)
          tftRusPrint(String(addMemoName[posMemoName - 1]),
                      (posMemoName * 12) - 12 + d, 157);
        if (posMemoName < 19)
          tftRusPrint(String(addMemoName[posMemoName + 1]),
                      (posMemoName * 12) + 12 + d, 157);
  ...
  if (tftPressed) {
    ...
    if (FirstLayer) {  //==================================================
                       // digit selection
      if ((currentMode == LSB || currentMode == USB || currentMode == CW) and
          (x > 139) and (x < 219) and (y > 25) and (y < 60)) {
        for (int n = 0; n <= lastdignum; n++) {
          if ((x > (dn[n].Xdignumos) + (dn[n].Xdignumnr)) and
              (x <
               ((dn[n].Xdignumos) + (dn[n].Xdignumsr) + (dn[n].Xdignumnr))) and
              (y > (dn[n].Ydignumos) and
      ...
      // battery selection
      if ((x > XVolInd + 161) and (x < XVolInd + 237) and (y > YVolInd - 1) and
          (y < YVolInd + 26)) {
        beeper.tick();
        delay(200);
        batVolt = !batVolt;
      ...
      // PRE selection
      if (x < (150 + (bfoOn * 80)) and y > 25 and y < 60 and !VOLbut and
          !AGCgainbut and !SQUELCHbut) {  // LWH - Added "and !SQUELCHbut"
        beeper.tick();
        delay(200);
        if (PREtap) {
          beeper.tick();
          ...
          if (PRE) {
            ...
            if (((band[bandIdx].bandType == MW_BAND_TYPE) or
                 (band[bandIdx].bandType == LW_BAND_TYPE)) and
                (currentMode == AM))
              ssIdxMW = tmpStep;
            if ((band[bandIdx].bandType == SW_BAND_TYPE) and
                (currentMode == AM))
              ssIdxAM = tmpStep;
    ...
    if (SecondLayer) {  //===============================================================
      if (Modebut) {
        if (x > 20 and x < 220 and y > 20 and y < 60 and !VOLbut) {
          ...
          if (n >= 0) {
            ...
            if ((currentMode != previousMode) and (currentMode == CW) and
                (CWShift == false)) {
              currentMode = USB;
              CWShift = true;
              currentBFO = currentBFO + 700;
      ...
      if (bandRETRObut) {
        ...
        for (int n = 0; n <= lastBandRetro; n++) {
          if ((screenV and x > bandRetro[n].xPosV and
               x < (bandRetro[n].xPosV + But_Width) and
               y > bandRetro[n].yPosV and
               y < (bandRetro[n].yPosV + But_Height)) or
              (!screenV and x > bandRetro[n].xPosH and
               x < (bandRetro[n].xPosH + But_Width) and
               y > bandRetro[n].yPosH and
               y < (bandRetro[n].yPosH + But_Height))) {
            beeper.tick();
            x = 0;
            y = 0;
      ...
      if (RETRObut) {
        if (y > 40 and y < 200) {
          ...
          if (bandRetro[RETROband].hardStep != bandRetro[RETROband].softStep or
              RETROband > 3) {
            if (bandHamRetro) {
              if (bfoOn)
                bfoOn = false;
              ...
              if (band[bandIdx].currentStep == bandRetro[RETROband].hardStep) {
                ...
                if (RETROband > 3 and band[bandIdx].currentStep ==
                                          bandRetro[RETROband].softStep) {
      ...
      if (MEMObut) {
        if (x > 20 and x < 220 and y > 20 and y < 60) {
          ...
          if (x > d and x < (240 + d) and y > 80 and y < (240 - d)) {
            if ((int(y / 40) - 3 + currentMemo) >= 0 and
                (int(y / 40) - 3 + currentMemo) <= lastMemoBank) {
              tftTransRect(d, int(y / 40) * 40, 240, 40, TFT_OLIVE);
              if (presetBank) {
                int tmpBand =
                    bandFreq(preset[int(y / 40) - 3 + currentMemo].presetIdx);
                if (tmpBand == 29 and
                    (preset[int(y / 40) - 3 + currentMemo].presetIdx < 153 or
                     preset[int(y / 40) - 3 + currentMemo].presetIdx > 30000)) {
                  beeper.error();
                  ...
                  if (tmpBand)
                    si4735.setFrequency(
                        preset[int(y / 40) - 3 + currentMemo].presetIdx);
                  else
                    si4735.setFrequency(
                        preset[int(y / 40) - 3 + currentMemo].presetIdx * 100);
                  band[bandIdx].currentFreq = si4735.getFrequency();
                  FreqDispl();
                }
              } else {
                if (MemoBank[int(y / 40) - 3 + currentMemo].freq < 153 or
                    MemoBank[int(y / 40) - 3 + currentMemo].freq > 30000) {
                  beeper.error();
                  Serial.println("Memo button 2");
                } else {
                  beeper.tick();
                  if (bandIdx != (MemoBank[int(y / 40) - 3 + currentMemo].band &
                                  0x1F) or
                      currentMode !=
                          trunc(MemoBank[int(y / 40) - 3 + currentMemo].band /
                                32)) {
                    bandIdx =
                        MemoBank[int(y / 40) - 3 + currentMemo].band & 0x1F;
                    currentMode = trunc(
                        MemoBank[int(y / 40) - 3 + currentMemo].band / 32);
                    BandSet();
                  }
                  si4735.setFrequency(
                      MemoBank[int(y / 40) - 3 + currentMemo].freq);
          ...
          if (n >= 0) {
            if (n == 0 and !presetBank) {  // EDIT
              if (MEMOadd or MEMOdel) {
                ...
                if (MemoBank[currentMemo].freq < 153 or
                    MemoBank[currentMemo].freq > 30000) {
                  drawButton(L_MEMO, 0, B_NORMAL);
                  beeper.error();
                  ...
                  for (int i = 0; i < 21; i++)
                    addMemoName[i] = MemoBank[currentMemo].Name[i];
                  posMemoName = 0;
      ...
      if (SETUPbut) {
        if (x > (!screenV * 40) and x < (240 + (!screenV * 40)) and y > 40 and
            y < 200) {
          beeper.tick();
      ...
      if (SCANbut and STEPbut == false) {
        ...
        if (y > 80 and y < 200) {
          ...
          if (SCANpause) {
            ...
            if (x < 40 and
                (currentScanFreq + int((deltaScanLine - 159 + d) * SCANstep)) >
                    band[bandIdx].minimumFreq)
              deltaScanLine -= (40 - x);
            if (x > (280 - (d * 2)) and
                (currentScanFreq + int((deltaScanLine + 160 - d) * SCANstep)) <
                    band[bandIdx].maximumFreq)
              deltaScanLine += (x - 280 + (d * 2));
            float tmpfreq =
                currentScanFreq + int((x - 159 + d + deltaScanLine) * SCANstep);
            int tmpline = currentScanLine;
            if (tmpfreq <= band[bandIdx].maximumFreq and
                tmpfreq >= band[bandIdx].minimumFreq) {
              currentScanLine = x;
            } else {
              if (tmpdelta != deltaScanLine) {
                if (tmpfreq > band[bandIdx].maximumFreq) {
                  deltaScanLine -=
                      ((tmpfreq - band[bandIdx].maximumFreq) / SCANstep);
                } else {
                  deltaScanLine +=
                      ((band[bandIdx].minimumFreq - tmpfreq) / SCANstep);
                }
                tmpfreq = currentScanFreq +
                          int((x - 159 + d + deltaScanLine) * SCANstep);
                currentScanLine = x;
              }
            }
            if (tmpline != currentScanLine or tmpdelta != deltaScanLine) {
              ...
              if (tmpdelta == deltaScanLine) {
                ...
                if (x < 40) {
                  ...
                  for (int i = 320 - (d * 2) - tmpdelta; i < (320 - (d * 2));
                       i++) {
                    ScanValueRSSI[i] = 198 + d;
                    ScanValueSNR[i] = 0;
                  }
            ...
            if (ScanEmpty) {
              ...
              if ((198 + d - ((198 + d - tmpMax) * tmpMid)) < (100 + (d / 2)))
                tmpMid = (98 + (d / 2)) / float(198 + d - tmpMax);
              if (tmpMid != 1 and tmpMid > 0) {
                if ((signalScale * tmpMid) > 10) tmpMid = 10 / signalScale;
                signalScale *= tmpMid;
                for (int i = 0; i < (320 - (d * 2)); i++)
                  ScanValueRSSI[i] =
                      198 + d - ((198 + d - ScanValueRSSI[i]) * tmpMid);
                drawSCANgraf(false);
                DrawSCANtxt(true);
              }
            }
      ...
      if (BroadBand) {
        ...
        if (x > 20 and x < 220 and y > 20 and y < 60 and !VOLbut) {
          ...
          if (n >= 0) {
            ...
            delay(400);
            BroadBand = false;
            if (bandIdx == 0 and currentAGCgain > 26)
              currentAGCgain = previousAGCgain =
                  26;  // currentAGCgain in FM max. 26
            si4735.setAM();
            delay(50);
      ...
      if (FREQbut) {
        ...
        if (x > 20 and x < 220 and y > 20 and y < 60 and !VOLbut) {
          ...
          if (n >= 0) {
            if (n != 9 or
                (!Decipoint and DisplayfreqNew >= (band[0].minimumFreq / 10) and
                 DisplayfreqNew <= (band[0].maximumFreq / 10))) {
              drawButton(L_FREQ, n, B_NORMAL);

              if ((n >= 0) and (n <= 8)) Freqcalq(n + 1);
              if (n == 10) Freqcalq(0);
              if (n == 9 and Decipoint == false and
                  DisplayfreqNew >= (band[0].minimumFreq / 10) and
                  DisplayfreqNew <= (band[0].maximumFreq / 10)) {
                Decipoint = true;
                fact = 10;
                Freqcalq(10);
              ...
              if (n == 11) {  // SET button
                ...
                if ((DisplayfreqNew > 30 and
                     DisplayfreqNew < (band[0].minimumFreq / 100)) or
                    (DisplayfreqNew > 108 and DisplayfreqNew < 153) or
                    DisplayfreqNew == 0) {
                  tft.setTextSize(2);
                  ...
                  if ((DisplayfreqNew >= (band[0].minimumFreq / 100)) and
                      (DisplayfreqNew <= (band[0].maximumFreq / 100))) {
                    // FM
                    ...
                    for (int q = 1; q <= lastBand; q++) {
                      if (((currentFrequency) >= band[q].minimumFreq) and
                          ((currentFrequency) <= band[q].maximumFreq)) {
                        bandIdx = q;
    ...
    if (ThirdLayer) {  //==================================================
      if ((currentMode == LSB || currentMode == USB || currentMode == CW) and
          (x > 139) and (x < 219) and (y > 25) and
          (y < 60)) {  // digit selection
        for (int n = 0; n <= lastdignum; n++) {
          if ((x > (dn[n].Xdignumos) + (dn[n].Xdignumnr)) and
              (x <
               ((dn[n].Xdignumos) + (dn[n].Xdignumsr) + (dn[n].Xdignumnr))) and
      ...
      if ((x > XVolInd + 161) and (x < XVolInd + 237) and (y > YVolInd - 1) and
          (y < YVolInd + 26)) {  // battery selection
        beeper.tick();
        delay(200);
        batVolt = !batVolt;
        elapsedBat = 0;
      ...
      if (n >= 0) {
        ...
        if (bfoOn and (n == B_SCAN or n == B_RETRO or n == B_FM or
                       n == B_MEMO or n == B_SETUP)) {
          bfoOn = false;
          if (n == B_FM)
        ...
        if (n == B_FM) {  //============================== FM button
          if (!PRESoff) {
            ...
            for (int i = 0; i <= lastPreset; i++)
              if ((preset[i].presetIdx * 100) >= band[0].minimumFreq and
                  (preset[i].presetIdx * 100) <= band[0].maximumFreq)
                flag = true;
            if (flag) {
              tft.fillRect(XFreqDispl, YFreqDispl + 20, 239, 65, TFT_DARKCYAN);
              previousPRES = -1;
        ...
        if (n == B_SCAN) {
          ...
          if (autoSCANstep) {
            float tmp =
                float(band[bandIdx].maximumFreq - band[bandIdx].minimumFreq) /
                (320 - (d * 2));
            ...
            if ((currentMinScanStep < minSCANstep) or
                (currentMode == USB or currentMode == LSB or currentMode == CW))
        ...
        if (n == B_SEEKUP) {
          ...
          if (currentMode != FM) {  // No FM
            if (band[bandIdx].bandType == MW_BAND_TYPE ||
              ...
              si4735.setSeekAmLimits(band[bandIdx].minimumFreq,
                                     band[bandIdx].maximumFreq);
            } else {
              bandIdx = 29;  // all sw
              if (seekAccuracy)
        ...
        //            DrawThla();
        //          }
        // ==================== Replaced it with the Squelch function
        // ============= LWH
        if (n == B_SQUELCH) {
          if (displayPower) {
            beeper.error();
  ...
  if (currentRetroFreq != si4735.getFrequency() and RETRObut) {
    if (bandHamRetro) {
      if ((currentRetroFreq - (currentBFO / 1000)) <
              band[bandHamRetro].minimumFreq or
          (currentRetroFreq - (currentBFO / 1000)) >
              band[bandHamRetro].maximumFreq) {
        bandHamRetro = 0;
  ...
  if (currentMode == LSB || currentMode == USB ||
      currentMode == CW)  // set BFO value in si4735
  {
  ...
  if (currentPRES != previousPRES and PRESbut) {
    ...
    while ((preset[currentPRES].presetIdx * 100) < band[0].minimumFreq or
           (preset[currentPRES].presetIdx * 100) > band[0].maximumFreq)
      currentPRES++;
    ...
    tft.setTextDatum(BL_DATUM);
    tft.setTextColor(TFT_WHITE, TFT_DARKCYAN);
    tft.drawString(String(preset[currentPRES].presetIdx, 2) + " MHz ", 1,
                   33);  // 5 , 83 posizione freq su schermata FM (preset)
...
void VOLbutoff() {
  //=======================================================================================
  if (((millis() - VOLbutOnTime) > MIN_ELAPSED_VOLbut_TIME * 30) and
      (VOLbut == true)) {
    VOLbut = false;
...
void DisplayRDS() {
  ...
  if ((currentMode == FM) and ((FirstLayer) or (ThirdLayer))) {
    if (currentFrequency != previousFrequency) {
      previousFrequency = currentFrequency;
      tft.fillRect(XFreqDispl + 70, YFreqDispl + 57, 115, 16,
                   TFT_BLACK);  // clear RDS stationName
      tft.fillRect(0, YFreqDispl + 75, 240, 16,
                   TFT_BLACK);  // clear RDS rdsMsg
                                //   tft.fillRect(0, YFreqDispl + 59, 80, 20,
                                //   TFT_BLACK);// clear RDS rdsTime
    }
...
void showtimeRSSI() {
  //=======================================================================================
  if ((millis() - elapsedRSSI) >
      MIN_ELAPSED_RSSI_TIME *
          RSSIfact)  // 150 * 1  = 150 msec refresh time RSSI
  {
    si4735.getCurrentReceivedSignalQuality();
...
void showRSSI() {
  ...
  if ((currentMode == FM) and ((FirstLayer) or (ThirdLayer) or
                               (SecondLayer and RETRObut and !RETROband))) {
    if (RETRObut) {
      int d = screenV * 80;
      ...
      tft.fillRect(XFreqDispl + 191, YFreqDispl + 60, 38, 11,
                   backGroundColor);  // STEREO MONO
  ...
  if ((currentMode == AM) or (currentMode == LSB) or (currentMode == USB) or
      (currentMode == CW) and ((FirstLayer) or (ThirdLayer) or
                               (SecondLayer and RETRObut and !RETROband))) {
    tft.setTextSize(1);
    tft.setTextColor(TFT_GREEN, TFT_BLACK);
    if ((FirstLayer or ThirdLayer) and
        !PRESbut) {  // dBuV and dB at freq. display
      tft.setTextDatum(TL_DATUM);
      tft.drawString("RSSI " + String(NewRSSI) + " dBuV ", XFreqDispl + 8,
                     YFreqDispl + 75);
      tft.setTextDatum(TR_DATUM);
      tft.drawString(" SNR " + String(NewSNR) + " dB", XFreqDispl + 180,
                     YFreqDispl + 75);
    }
  }
  VOLbutoff();
...
void encoderCheck() {
  ...
  if (rotaryEncoderResult.direction != RotaryEncoder::Direction::NONE) {
    ...
    if (SCANbut and VOLbut == false and bfoOn == false) {
      ...
      float tmpcurline =
          (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP)
              ? (currentScanLine +
                 (float(band[bandIdx].currentStep) / SCANstep))
              : (currentScanLine -
                 (float(band[bandIdx].currentStep) / SCANstep));
      if ((rotaryEncoderResult.direction == RotaryEncoder::Direction::UP and
           (currentScanFreq + ((tmpcurline - 159 + d + deltaScanLine) *
                               SCANstep)) <= band[bandIdx].maximumFreq) or
          (rotaryEncoderResult.direction != RotaryEncoder::Direction::UP and
           (currentScanFreq + ((tmpcurline - 159 + d + deltaScanLine) *
                               SCANstep)) >= band[bandIdx].minimumFreq)) {
        int tmpline = currentScanLine;
        ...
        if (trunc(currentScanLine) < 0) {
          ...
          if (trunc(currentScanLine) > (319 - (d * 2))) {
            for (int i = currentScanLine - 319 + (d * 2); i < (320 - (d * 2));
                 i++) {
              ScanValueRSSI[int(i - currentScanLine + 319 - (d * 2))] =
                  ScanValueRSSI[i];
              ScanValueSNR[int(i - currentScanLine + 319 - (d * 2))] =
                  ScanValueSNR[i];
            }
            for (int i = 639 - (d * 4) - currentScanLine; i < (320 - (d * 2));
                 i++) {
              ScanValueRSSI[i] = 198 + d;
              ScanValueSNR[i] = 0;
            }
      ...
      if (RETRObut) {
        ...
        currentBFOmanu =
            (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP)
                ? (currentBFOmanu + currentBFOStep)
                : (currentBFOmanu - currentBFOStep);
        if (currentBFOmanu > 999 or currentBFOmanu < -999)
          currentBFOmanu = previousBFOmanu;
      }
      mainpurp = false;
#endif
    }
    ...
    // ======================== Change the Squelch Setting
    // ========================= LWH
    if (SQUELCHbut) {  // Squelch control
      currentSquelch =
          (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP)
              ? (currentSquelch + currentSQUELCHStep)
              : (currentSquelch - currentSQUELCHStep);
    ...
    if (AGCgainbut) {  // AGC gain control
      currentAGCgain =
          (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP)
              ? (currentAGCgain + currentAGCgainStep)
              : (currentAGCgain - currentAGCgainStep);
      mainpurp = false;
    }
      ...
      if (MEMOadd) {
        charMemoName =
            (rotaryEncoderResult.direction == RotaryEncoder::Direction::UP)
                ? (charMemoName + 1)
                : (charMemoName - 1);
        if (charMemoName == 144 and
            rotaryEncoderResult.direction == RotaryEncoder::Direction::UP)
          charMemoName = 32;
        if (charMemoName == 128 and
            rotaryEncoderResult.direction == RotaryEncoder::Direction::UP)
          charMemoName = 144;
        if (charMemoName == 192) charMemoName = 128;
        if (charMemoName == 127 and
            rotaryEncoderResult.direction != RotaryEncoder::Direction::UP)
          charMemoName = 191;
        if (charMemoName == 143 and
            rotaryEncoderResult.direction != RotaryEncoder::Direction::UP)
          charMemoName = 127;
    ...
    if (!PRESbut and !RETRObut and !VOLbut and !AGCgainbut and !bright and
        !cityRETRObut and !bandRETRObut and !SETUPbut and !MEMObut and
        !SQUELCHbut)
...
void encoderButtonCheck() {
  //=======================================================================================
  // Encoder button
  if (digitalRead(PIN_ENCODER_SW) == LOW and
      (ThirdLayer or FirstLayer or SecondLayer)) {
    beeper.tick();
    ...
    if (digitalRead(PIN_ENCODER_SW) == LOW) {
      ...
      if (tftPressed) {
        ...
        if ((currentMode == LSB || currentMode == USB || currentMode == CW) and
          ...
          if (currentMode == LSB || currentMode == USB) {
            ...
            int i = 1;
            while (currentRetroFreq < band[i].minimumFreq or
                   currentRetroFreq > band[i].maximumFreq)
              i++;
      ...
      if (digitalRead(PIN_ENCODER_SW) == LOW) {
        ...
        if (digitalRead(PIN_ENCODER_SW) == LOW) {
          ...
          if ((currentMode != LSB) and (currentMode != USB) and
              (currentMode != CW) and (ThirdLayer or FirstLayer)) {
            SEEK = true;
            SEEKdispl(1);
        ...
        if (MEMObut) {
          if (MEMOadd) {
            ...
            if (presetBank) {
              ...
              if (tmpBand == 29 and (preset[currentMemo].presetIdx < 153 or
                                     preset[currentMemo].presetIdx > 30000)) {
                beeper.error();
              ...
              if (MemoBank[currentMemo].freq < 153 or
                ...
                if (bandIdx != (MemoBank[currentMemo].band & 0x1F) or
...
#endif
        if (mainpurp) {
          // ================== Toggle between the volume control and the
          // squelch control ========== LWH
...
void setStep() {
  ...
  if (bfoOn &&
      (currentMode == LSB || currentMode == USB || currentMode == CW)) {
    if (currentBFOStep == 1)
      currentBFOStep = 10;
...
void subrstatus() {
  ...
  tft.drawString("Mode     : " + String(bandModeDesc[band[bandIdx].prefmod]), 5,
                 10);
  if (currentMode != FM)
    tft.drawString("Freq.    : " + String(currentFrequency, 0) + " KHz", 5, 20);
  else
    tft.drawString("Freq.    : " + String(currentFrequency / 100, 1) + " MHz",
                   5, 20);
  si4735.getCurrentReceivedSignalQuality();
  tft.drawString("RSSI     : " + String(si4735.getCurrentRSSI()) + "dBuV", 5,
                 30);
  ...
  si4735.getAutomaticGainControl();
  si4735.getCurrentReceivedSignalQuality();
  tft.drawString("LNA GAIN index: " + String(si4735.getAgcGainIndex()) + "/" +
                     String(currentAGCgain),
                 5, 60);
  ...
  tft.drawString("AVC max GAIN  : " + String(si4735.getCurrentAvcAmMaxGain()),
                 5, 100);
  tft.drawString("Ant. Cap = " + String(si4735.getAntennaTuningCapacitor()), 5,
                 110);

  tft.setTextColor(TFT_WHITE, TFT_BLACK);
  tft.drawString("Band          : " + String(bandIdx) + "  " +
                     String(band[bandIdx].bandName),
                 5, 120);
  tft.drawString("Bandwidth SSB : " + String(bandwidthSSB[bwIdxSSB]) + " KHz",
                 5, 130);
  tft.drawString("Bandwidth AM  : " + String(bandwidthAM[bwIdxAM]) + " KHz", 5,
                 140);
  tft.drawString("Bandwidth FM  : " + String(bandwidthFM[bwIdxFM]) + " KHz", 5,
                 150);
  ...
  tft.drawString("EEPROM SIZE    : " + String(EEPROM_SIZE) + " byte | FREE: " +
                     String(EEPROM_SIZE - offsetEEPROM - sizeof(MemoBank) -
                            sizeof(storage)) +
                     " byte",
                 5, 210);
  tft.drawString("EEPROM storage : " + String(sizeof(storage)) +
                     " byte. Offset: " + String(offsetEEPROM),
                 5, 220);
  tft.drawString("EEPROM MemoBank: " + String(sizeof(MemoBank)) +
                     " byte. Offset: " + String(offsetMemoEEPROM),
                 5, 230);
...
void FreqDispl() {
  ...
  if (FirstLayer or ThirdLayer or SecondLayer) {
    int d = 0;
    if ((SCANbut or HamBand or Modebut or STEPbut or BandWidth or MEMObut) and
        !screenV)
      d = 40;
    currentFrequency = si4735.getFrequency();
    if (!FREQbut and !HamBand and !Modebut and !BandWidth and !BroadBand and
        !SCANbut and !MEMObut and !STEPbut) {
      AGCfreqdisp();
      BFOStepdisp();
    }
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    ...
    if ((VOLbut) or (AGCgainbut) or (SQUELCHbut)) {  // LWH - Added SQUELCHbut
      ...
      //============ Squelch Setting ============ LWH
      if (SQUELCHbut) {
        tft.drawString(String(currentSquelch), XFreqDispl + 80 + d,
                       YFreqDispl + 53);
        tft.setTextSize(2);
      ...
      // ========================================
      if (VOLbut) {
        tft.drawString(String(currentVOL), XFreqDispl + 84 + d,
                       YFreqDispl + 53);
        tft.setTextSize(2);
        tft.drawString("VOLUME", XFreqDispl + 160 + d, YFreqDispl + 53);
      }
      if (AGCgainbut) {
        tft.drawString(String(currentAGCgain), XFreqDispl + 60,
                       YFreqDispl + 53);
        tft.setTextSize(2);
      ...
      if (currentMode == LSB || currentMode == USB || currentMode == CW) {
        ...
        if (!bfoOn or bfoTr) {
          ...
          tft.setTextSize(2);
          if (FREQbut or HamBand or Modebut or BandWidth or BroadBand or
              SCANbut or MEMObut or STEPbut) {
          ...
          if (bfoOn == false and (FREQbut or HamBand or Modebut or BandWidth or
                                  BroadBand or SCANbut or MEMObut) == false) {
            tft.fillRect(XFreqDispl + 141 + d, YFreqDispl + 60, 81, 5,
                         TFT_BLACK);
            if (freqstepnr == 0)
              tft.fillRect(XFreqDispl + 141 + d, YFreqDispl + 60, 21, 5,
                           TFT_ORANGE);
            if (freqstepnr == 1)
              tft.fillRect(XFreqDispl + 171 + d, YFreqDispl + 60, 21, 5,
                           TFT_ORANGE);
            if (freqstepnr == 2)
              tft.fillRect(XFreqDispl + 200 + d, YFreqDispl + 60, 21, 5,
                           TFT_ORANGE);
          }
        }
        if (bfoOn) {
          ...
          tft.fillRect(XFreqDispl + 156 + d, YFreqDispl + 21, 42, 20,
                       TFT_ORANGE);
...
void FreqDraw(float freq, int d) {
  ...
  if (currentMode == FM) {
    ...
    if (Saver) {
      ...
      tft.setTextColor(TFT_YELLOW, TFT_BLACK);
      tft.drawString("MHz", XFreqDispl + 215 + d,
                     YFreqDispl + 50);  // 60 posizione scritta Mhz in FM
    }
  } else {
    if (band[bandIdx].bandType == MW_BAND_TYPE ||
        band[bandIdx].bandType == LW_BAND_TYPE) {
...
bool checkStopSeeking() {
  ...
  return rotaryEncoderResult.direction != RotaryEncoder::Direction::NONE or
         tft.getTouch(&x, &y) or
         digitalRead(PIN_ENCODER_SW) ==
             LOW;  // returns true if the user rotates the encoder or touches on
                   // screen
}
...
void DrawDispl() {
  ...
  if (currentMode == FM) {
    tft.drawString(String((band[bandIdx].currentStep) * 10) + "KHz",
                   XFreqDispl + 220, YFreqDispl + 15);
  } else
    tft.drawString(String(band[bandIdx].currentStep) + "KHz", XFreqDispl + 220,
                   YFreqDispl + 15);
...
void AGCfreqdisp() {
  ...
  if (AGCgain > 1) {
    if (currentAGCgain > 9)
      tft.drawString("ATT" + String(currentAGCgain), XFreqDispl + 60,
                     YFreqDispl + 15);
    else
      tft.drawString("ATT " + String(currentAGCgain), XFreqDispl + 60,
                     YFreqDispl + 15);
...
void BFOStepdisp() {
  ...
  if (bfoOn) {
#ifdef IhaveCrystal
    tft.drawString(String(currentBFOStep) + " Hz", XFreqDispl + 20,
                   YFreqDispl + 15);
#endif
  } else {
...
void MuteAud() {
  ...
  if (((millis() - elapsedAudMut) > MIN_ELAPSED_AudMut_TIME) and
      (AudioMut = true)) {
    AudioMut = false;
...
void pauseSCAN() {
  ...
  if (SCANpause) {
    ...
    if (band[bandIdx].bandType == MW_BAND_TYPE or
      ...
      si4735.setFrequencyStep(ssIdxAM);
    setFreq(currentScanFreq +
            int((currentScanLine - 159 + d + deltaScanLine) * SCANstep));
    AGCgain = ScanAGC;
    checkAGC();
    ...
    posScanFreq =
        currentScanFreq + int((deltaScanLine - 159 + d + posScan) * SCANstep);
    setFreq(posScanFreq);
    if (currentMode == LSB or currentMode == USB or currentMode == CW)
      band[bandIdx].lastBFO = currentBFO = 0;
    AGCgain = 0;
...
void DisplaySCANsignal() {
  ...
  tft.setTextDatum(BL_DATUM);
  if (SCANpause)
    tft.drawString("RSSI " + String(si4735.getCurrentRSSI()) + " dBuV ",
                   130 - d, 70);
  else
    tft.drawString(
        "RSSI " +
            String((198 - ScanValueRSSI[int(currentScanLine)]) / signalScale,
                   0) +
            " dBuV ",
        130 - d, 70);
  tft.setTextColor(TFT_ORANGE, TFT_GREY);
  if (SCANpause)
    tft.drawString(" SNR " + String(si4735.getCurrentSNR()) + " dB ", 130 - d,
                   80);
  else
    tft.drawString(
        " SNR " + String(ScanValueSNR[int(currentScanLine)]) + " dB ", 130 - d,
        80);
}
...
void drawSCAN() {
  ...
  setFreq(
      currentScanFreq +
      int((currentScanLine - 159 + (screenV * 40) + deltaScanLine) * SCANstep));
  FreqDispl();
...
void drawSCANline(int n) {
  ...
  if (n == int(currentScanLine)) {
    tft.drawLine(int(currentScanLine), 81, int(currentScanLine), 198 + d,
                 TFT_RED);
    DisplaySCANsignal();
  } else {
    ...
    // overflow BAND
    if ((currentScanFreq + ((n - 159 + d + deltaScanLine) * SCANstep)) >
        band[bandIdx].maximumFreq) {
      colf = TFT_GREY;
    ...
    if (!ScanScaleLine[n]) {
      if (SCANstep <= 2) {
        if (SCANstep == 2) {
          if ((frq + 50 - (int((frq + 50) / 100) * 100)) < 2)
            ScanScaleLine[n] = 3;
        } else {
...
void DisplaySCAN() {
  ...
  bool setf = false;
  posScan =
      (int(posScanFreq - currentScanFreq) / SCANstep) + 159 - d - deltaScanLine;
  ...
  if (setf) {
    setFreq(currentScanFreq +
            int((deltaScanLine - 159 + d + posScan) * SCANstep));
  } else {
    ...
    if (ScanValueSNR[posScan] >= ScanMarkSNR and posScan > ScanBeginBand and
        posScan < ScanEndBand)
      ScanMark[posScan] = true;
    if (!Saver) {
    ...
    if (SCANstep < 1) {
      for (int i = 1; i < 1 / SCANstep; i++) {
        posScan++;
        if (posScan < (320 - (d * 2))) {
          if ((currentMode == LSB or currentMode == USB or
               currentMode == CW) and
              !ScanEmpty) {
      ...
      if ((currentMode == LSB or currentMode == USB or currentMode == CW) and
          !ScanEmpty)
        si4735.setSSBBfo(0);
    }
    posScanLast = posScan;
    ...
    if (ScanEmpty)
      setFreq(currentScanFreq +
              int((deltaScanLine - 159 + d + posScan + 1) * SCANstep));
    else
      freqUp();
  }
}
...
void setFreq(float f) {
  ...
  if (currentMode == LSB or currentMode == USB or currentMode == CW)
    si4735.setAutomaticGainControl(1, 0);  // AGC disabled
}
...
void DrawSCANtxt(bool all) {
  ...
  if ((ScanEndBand < (315 - d)) and
          ((posScan > (ScanEndBand + 5)) and (posScan < (ScanEndBand + 45))) or
      all) {
  ...
  if ((ScanBeginBand > 5) and ((posScan > (ScanBeginBand - 43)) and
                               (posScan < (ScanBeginBand - 3))) or
      all) {
  ...
  if (posScan < 60 or all) {
    ...
    tft.setTextDatum(BL_DATUM);
    if (currentMode == FM)
      tft.drawString(String((currentScanFreq +
                             (SCANstep * (deltaScanLine - 159 + (d / 2)))) /
                            100) +
                         " MHz ",
                     0, 90);
    else
      tft.drawString(String(int(currentScanFreq +
                                (SCANstep * (deltaScanLine - 159 + (d / 2))))) +
                         " KHz ",
                     0, 90);
    // scale
    ...
    tft.setTextColor(TFT_YELLOW, TFT_BLACK);
    if (currentMode == FM and SCANstep > 4)
      tft.drawString("10 MHz", 0, 195 + (d / 2));
    if ((currentMode == FM and SCANstep == 4) or
        (currentMode != FM and SCANstep > 4))
      tft.drawString("1 MHz", 0, 195 + (d / 2));
    if (currentMode == FM and SCANstep == 2)
      tft.drawString("500 KHz", 0, 195 + (d / 2));
    if ((currentMode == FM and SCANstep < 2) or
        (currentMode != FM and SCANstep == 4))
      tft.drawString("100 KHz", 0, 195 + (d / 2));
    if (currentMode != FM and SCANstep == 2)
      tft.drawString("50 KHz", 0, 195 + (d / 2));
    if (currentMode != FM and SCANstep < 2)
      tft.drawString("10 KHz", 0, 195 + (d / 2));
  }
  if (posScan > (240 - d) or all) {
    ...
    if (currentMode == FM)
      tft.drawString(" " +
                         String((currentScanFreq +
                                 (SCANstep * (160 - (d / 2) + deltaScanLine))) /
                                100) +
                         " MHz",
                     319 - d, 90);
    else
...
void DrawSCANind() {
  ...
  if (currentMode == FM)
    tft.drawString(String((band[bandIdx].currentStep) * 10) + " KHz", d + 25,
                   75);
  else
...
void initRetro() {
  //=======================================================================================
  drawRETRO();
  if (bandIdx != bandRetro[RETROband].band or
      currentMode != bandMode[bandIdx]) {
    bandIdx = bandRetro[RETROband].band;
    currentMode = bandMode[bandIdx];
    BandSet();
  }
  if (bandRetro[RETROband].currentFreq < bandRetro[RETROband].minimumFreq or
      bandRetro[RETROband].currentFreq > bandRetro[RETROband].maximumFreq)
...
void drawRETROscale() {
  ...
  if (band[bandIdx].currentStep == bandRetro[RETROband].softStep and
      bandRetro[RETROband].hardStep != bandRetro[RETROband].softStep)
    color = 0xFE10;
  if (band[bandIdx].currentStep != bandRetro[RETROband].softStep and
      band[bandIdx].currentStep != bandRetro[RETROband].hardStep)
    color = 0xFF14;
  ...
  float freq = (Displayfreq - (trunc(Displayfreq / bandRetro[RETROband].mark) *
                               bandRetro[RETROband].mark)) *
               currentRetroScale;
  tft.fillRect(0, 112, 320 - d, 16, color);

  for (float i = tmp - freq - tmpMark; i < (320 - d + (tmpMark / 2));
       i += tmpMark) {
    freq = Displayfreq + ((i - 160 + (d / 2)) / currentRetroScale);
    if (freq > bandRetro[RETROband].maximumFreq or
        freq < bandRetro[RETROband].minimumFreq) {
      for (int n = 0; n < (tmpMark / 2); n += 4)
        tft.fillRect(i + n, 112, 2, 15, TFT_BLACK);
    } else {
      tftRusPrint(String(freq, 0), i, 126);
    }
    freq =
        Displayfreq + ((i + (tmpMark / 2) - 160 + (d / 2)) / currentRetroScale);
    if (freq > bandRetro[RETROband].maximumFreq or
        (freq + (bandRetro[RETROband].mark / 2)) <
            bandRetro[RETROband].minimumFreq) {
      for (int n = 0; n < (tmpMark / 2); n += 4)
        tft.fillRect(i + n + (tmpMark / 2), 112, 2, 15, TFT_BLACK);
    } else {
  ...
  if (RETROband > 3) {
    ...
    while (i < 29) {
      if (bandMode[i] == LSB or bandMode[i] == USB) {
        if (band[i].currentFreq >= bandRetro[RETROband].minimumFreq and
            band[i].currentFreq <= bandRetro[RETROband].maximumFreq) {
          int minPos =
              ((band[i].minimumFreq - bandRetro[RETROband].currentFreq) *
               currentRetroScale) +
              159 - (d / 2);
          int maxPos =
              ((band[i].maximumFreq - bandRetro[RETROband].currentFreq) *
               currentRetroScale) +
              159 - (d / 2);
          if (maxPos > -(currentRetroScale * 10) and
              minPos < (319 - d + (currentRetroScale * 10))) {
            if (minPos < -11) minPos = -11;
            if (maxPos > 330) maxPos = 330 - d;
            tft.fillRect(minPos, RetroStationPos[5] - 2, maxPos - minPos + 1, 9,
                         TFT_BROWN);
          }
        }
      }
  ...
  for (int i = 0; i <= lastPreset; i++) {
    if (preset[i].presetIdx >= bandRetro[RETROband].minimumFreq and
        preset[i].presetIdx <= bandRetro[RETROband].maximumFreq) {
      freq = (((preset[i].presetIdx) - Displayfreq) * currentRetroScale) + 160 -
             (d / 2);
      int tmplen = tftRusLength(preset[i].PresetName) * 8;
      if (freq > -20 and freq < tmplen + 350 - d) {
        tft.fillRect(freq - tmplen - 40, preset[i].presetPos - 3, tmplen + 60,
                     11, TFT_BLACK);
        ...
        tftRusPrint(String(preset[i].PresetName), freq - 20,
                    preset[i].presetPos + 9);
      }
    }
  }
...
void drawRetroCity() {
  ...
  if (id < 0 or id > lastGroup) {
    ...
    for (int i = 0; i < 7; i++) {
      if (!screenV) {
        for (int n = 1; n <= 20; n++) {
          tft.fillRect(40 - (n * 2), (i * 30) + 20, 2, 30,
                       ((int(n / 2) * 4096) + (n * 32)));
          tft.fillRect((n * 2) + 278, (i * 30) + 20, 2, 30,
                       ((int(n / 2) * 4096) + (n * 32)));
        }
      }
      ...
      if ((id + i - 3) >= 0 and (id + i - 3) <= lastGroup) {
        ...
        tftRusPrint(String(group[id + i - 3].PresetName), 120 + d,
                    (i * 30) + 45);
      }
    }
  }
...
void drawRetroBandBut() {
  ...
  for (int n = 0; n <= lastBandRetro; n++) {
    ...
    if (n == RETROband) {
      ...
      if (langRetroEN) {
        if (screenV)
          tftRusPrint(String(bandRetro[n].BandName),
                      bandRetro[n].xPosV + (But_Width / 2) + 3,
                      bandRetro[n].yPosV + (But_Height / 2) + 9);
        else
          tftRusPrint(String(bandRetro[n].BandName),
                      bandRetro[n].xPosH + (But_Width / 2) + 3,
                      bandRetro[n].yPosH + (But_Height / 2) + 9);
      } else {
        if (screenV)
          tftRusPrint(String(bandRetro[n].BandNameRU),
                      bandRetro[n].xPosV + (But_Width / 2) + 3,
                      bandRetro[n].yPosV + (But_Height / 2) + 9);
        else
          tftRusPrint(String(bandRetro[n].BandNameRU),
    ...
    if (langRetroEN) {
      if (screenV)
        tftRusPrint(String(bandRetro[n].BandName),
                    bandRetro[n].xPosV + (But_Width / 2) + 2,
                    bandRetro[n].yPosV + (But_Height / 2) + 8);
      else
        tftRusPrint(String(bandRetro[n].BandName),
                    bandRetro[n].xPosH + (But_Width / 2) + 2,
                    bandRetro[n].yPosH + (But_Height / 2) + 8);
    } else {
      if (screenV)
        tftRusPrint(String(bandRetro[n].BandNameRU),
                    bandRetro[n].xPosV + (But_Width / 2) + 2,
    ...
    if ((langRetroEN and bandRetro[n].RetroBandTime != "") or
        (!langRetroEN and bandRetro[n].RetroBandTimeRU != "")) {
      if (screenV)
        tft.drawRect(bandRetro[n].xPosV, bandRetro[n].yPosV, 220, 40,
                     TFT_OLIVE);
      else
        tft.drawRect(bandRetro[n].xPosH, bandRetro[n].yPosH, 220, 40,
                     TFT_OLIVE);
    }
    tftRusSetFont(T1012_T);
    tftRusSetStyle(NRG_T);
    ...
    if (langRetroEN) {
      if (screenV)
        tftRusPrint(String(bandRetro[n].RetroBandTime), bandRetro[n].xPosV + 90,
                    bandRetro[n].yPosV + 27);
      else
        tftRusPrint(String(bandRetro[n].RetroBandTime), bandRetro[n].xPosH + 90,
                    bandRetro[n].yPosH + 27);
    } else {
      if (screenV)
        tftRusPrint(String(bandRetro[n].RetroBandTimeRU),
                    bandRetro[n].xPosV + 90, bandRetro[n].yPosV + 27);
      else
        tftRusPrint(String(bandRetro[n].RetroBandTimeRU),
                    bandRetro[n].xPosH + 90, bandRetro[n].yPosH + 27);
    }
  }
  spr.deleteSprite();
}
  ...
  for (int i = -1; i < (2 + screenV); i++) {
    ...
    if (presetBank) {
      if ((currentMemo + i) >= 0 and (currentMemo + i) <= lastPreset) {
        ...
        tftRusSetCut(0, 20);
        tftRusWidth = 12;
        tftRusPrint(String(preset[currentMemo + i].PresetName), d,
                    (i * 40) + 157);
        tftRusSetCut(0, 0);
        tftRusSetDatum(BR_T);
        tftRusSetColor(TFT_CYAN, TFT_TRANS);
        if (preset[currentMemo + i].presetIdx < 109)
          tftRusPrint(String(preset[currentMemo + i].presetIdx, 2) + " MHz",
                      d + 155, (i * 40) + 141);
        else
          tftRusPrint(String(preset[currentMemo + i].presetIdx, 0) + " KHz",
                      d + 155, (i * 40) + 141);
        // BAND
        if (i)
        ...
        tft.drawString(
            band[bandFreq(preset[currentMemo + i].presetIdx)].bandName, d + 175,
            (i * 40) + 137);
        ...
        tft.drawString(
            bandModeDesc[bandMode[bandFreq(preset[currentMemo + i].presetIdx)]],
            d + 215, (i * 40) + 137);
      ...
      if ((currentMemo + i) >= 0 and (currentMemo + i) <= lastMemoBank) {
        ...
        tftRusSetColor(TFT_YELLOW, TFT_TRANS);
        tftRusPrint(String(currentMemo + i + 1), d, (i * 40) + 141);
        if ((MemoBank[currentMemo + i].freq < 153 or
             MemoBank[currentMemo + i].freq > 30000) and
            !(!i and MEMOadd)) {
          if (!i and MEMOdel) {
            MEMOdel = false;
            beeper.error();
          ...
          if (MEMOadd and !i)
            ...
            tftRusPrint(String(MemoBank[currentMemo + i].Name), d,
                        (i * 40) + 157);
          tftRusSetCut(0, 0);
          tftRusSetDatum(BR_T);
          tftRusSetColor(TFT_CYAN, TFT_TRANS);
          if (MEMOadd and !i) {
            if (addMemoBand == FM)
              tftRusPrint(String(float(addMemoFreq) / 100, 2) + " MHz", d + 155,
                          141);
            else
            ...
            if (MemoBank[currentMemo + i].band == FM)
              tftRusPrint(
                  String(float(MemoBank[currentMemo + i].freq) / 100, 2) +
                      " MHz",
                  d + 155, (i * 40) + 141);
            else
              tftRusPrint(String(MemoBank[currentMemo + i].freq) + " KHz",
                          d + 155, (i * 40) + 141);
          }
          // BAND
          if (i)
          ...
          if (MEMOadd and !i)
            ...
            tft.drawString(
                band[(MemoBank[currentMemo + i].band & 0x1F)].bandName, d + 175,
                (i * 40) + 137);
          tft.drawRect(d + 155, (i * 40) + 124, 39, 16, 2031);
          // MODE
          if (i)
          ...
          if (MEMOadd and !i)
            ...
            tft.drawString(
                bandModeDesc[int(MemoBank[currentMemo + i].band / 32)], d + 215,
                (i * 40) + 137);
          tft.drawRect(d + 195, (i * 40) + 124, 39, 16, TFT_YELLOW);
        }
      }
...
void displSETUP() {
  ...
  switch (pageSetup) {
    case 0:
      ...
      displSETUPitem("Seek AM 1 KHz    ", 120, prevseekAccuracy,
                     (seekAccuracy != prevseekAccuracy));
      break;
    case 1:
      tft.drawString("USAGE", 240 + d, 40);
      displSETUPitem("RDS only FMbutton", 40, !prevRDSalways,
                     (RDSalways != prevRDSalways));
      displSETUPitem("Digit backlight  ", 80, prevdigitLigth,
                     (digitLigth != prevdigitLigth));
      displSETUPitem("Memo in preset   ", 120, prevmemoPreset,
                     (memoPreset != prevmemoPreset));
      displSETUPitem("Retro language RU", 160, !prevlangRetroEN,
                     (langRetroEN != prevlangRetroEN));
      break;
    case 2:
      tft.drawString("DISPLAY", 240 + d, 40);
      displSETUPitem("Screen saver     ", 40, prevsaverOn,
                     (saverOn != prevsaverOn));
      displSETUPitem("Display light off", 80, prevdisplayOff,
                     (displayOff != prevdisplayOff));
      displSETUPitemValue("Saver time in min", 120, String(prevsaverTime),
                          (saverTime != prevsaverTime));
      displSETUPitem("Vertical screen  ", 160, prevscreenV,
                     (screenV != prevscreenV));
      break;
    case 3:
      tft.drawString("SCAN", 240 + d, 40);
      displSETUPitemValue("Minimum scale    ", 40,
                          String("x" + String(int(1 / prevminSCANstep))),
                          (minSCANstep != prevminSCANstep));
      displSETUPitemValue("Maximum scale    ", 80,
                          String("1:" + String(int(prevmaxSCANstep))),
                          (maxSCANstep != prevmaxSCANstep));
      displSETUPitem("Auto scale       ", 120, prevautoSCANstep,
                     (autoSCANstep != prevautoSCANstep));
      displSETUPitem("Scan accuracy    ", 160, prevSCANaccuracy,
                     (SCANaccuracy != prevSCANaccuracy));
      break;
    case 4:
      tft.drawString("HARDWARE", 240 + d, 40);
      displSETUPitem("Battery show     ", 80, prevbatShow,
                     (batShow != prevbatShow));
      displSETUPitem("Beeper On        ", 120, prevbeeperOn,
                     (beeperOn != prevbeeperOn));
      displSETUPitem("Bright disp power", 160, prevdisplayPower,
                     (displayPower != prevdisplayPower));
      break;
    case 5:
      tft.drawString("DEFAULT", 240 + d, 40);
      displSETUPitem("Load default Memo", 80, prevloadMemory,
                     (loadMemory != prevloadMemory));
      displSETUPitem("Reset to factory ", 120, prevloadDefault,
...
void saveSETUP() {
  //=======================================================================================
  if (VHFon != prevVHFon or langRetroEN != prevlangRetroEN or
      beeperOn != prevbeeperOn or digitLigth != prevdigitLigth or
      loadMemory != prevloadMemory or batShow != prevbatShow or
      memoPreset != prevmemoPreset or loadDefault != prevloadDefault or
      saverOn != prevsaverOn or saverTime != prevsaverTime or
      screenV != prevscreenV or displayOff != prevdisplayOff or
      minSCANstep != prevminSCANstep or maxSCANstep != prevmaxSCANstep or
      autoSCANstep != prevautoSCANstep or SCANaccuracy != prevSCANaccuracy or
      displayPower != prevdisplayPower or RDSalways != prevRDSalways or
      seekAccuracy != prevseekAccuracy) {
...
//=======================================================================================
String
presetNameLoad() {  // loading preset name ---------------------------------
  //=======================================================================================
  String ret = "";
  for (int i = 0; i <= lastGroup; i++)
  ...
  if (memoPreset and !presetBank) {
    for (int i = 0; i <= lastMemoBank; i++) {
      if (MemoBank[i].freq >= 153 and MemoBank[i].freq <= 30000) {
        ...
        int j = 19;
        while (j > 0 and (MemoBank[i].Name[j] == char(32) or
                          MemoBank[i].Name[j] == char(0)))
          j--;
...
void presetSetPos() {  // set position fm station on retro scale
  ...
  for (int i = 0; i <= lastPreset; i++) {
    if (preset[i].presetIdx >= bandRetro[RETROband].minimumFreq and
      ...
      int j = 0;
      while (j < 12) {
        if (tmpPos[j] <= (((preset[i].presetIdx) * currentRetroScale) -
                          ((tftRusLength(preset[i].PresetName) * 8) + 60)))
          j = 12;
        j++;
      }
      if (j == 12) {
        ...
        while (tmpPos[y] > (((preset[i].presetIdx) * currentRetroScale) -
                            ((tftRusLength(preset[i].PresetName) * 8) + 60))) {
          y += random(1, 3);
          if (y > 11) y = random(3);
        }
...
void tftRusPrint(String text, int x, int y) {
  ...
  for (int i = 0; i < text.length(); i++) {
    ...
    if (textLength >= tftRusBeginChar and
        ((textLength - tftRusBeginChar) < tftRusContChar or !tftRusContChar))
      ascii[textLength - tftRusBeginChar] = char(text[i]) - 32;
    textLength++;
  }
  textLength -= tftRusBeginChar;
  if (textLength > tftRusContChar and tftRusContChar)
    textLength = tftRusContChar;
      ...
      for (int xx = 0; xx < tftRusWidth; xx++) {
        ...
        for (int yb = 0; yb < 2; yb++) {
          ...
          if (tftRusFont) {
            ...
            if ((tftRusStyle & 0x01) and xx > 0 and xx < 12)
              byp = Tahoma15x16[(ascii[i] * 22) + ((xx - 1) * 2) + yb];
          } else {
            if (xx > 6) {
              by = 0x00;
            } else {
            ...
            if ((tftRusStyle & 0x01) and xx > 0 and xx < 8)
              byp = Tahoma10x12[(ascii[i] * 14) + ((xx - 1) * 2) + yb];
          }

          for (int yy = 0; yy < (8 - (yb * 4 * (1 - tftRusFont)) -
                                 (tftRusBottomCut * (yb * (2 + tftRusFont))));
               yy++) {
            bool bi = (by >> yy) & 0x01;
            if ((tftRusStyle & 0x01) and xx > 0 and xx < (8 + (4 * tftRusFont)))
              bi = bi | ((byp >> yy) & 0x01);
            for (int sy = 0; sy < tftRusSize; sy++) {
              for (int sx = 0; sx < tftRusSize; sx++) {
                if (bi) {
                  if (tftRusColor >= 0)
                    tft.drawPixel(
                        x + (i * tftRusSize * tftRusWidth) + (xx * tftRusSize) +
                            sx + cd,
                        y + (yb * tftRusSize * 8) + (yy * tftRusSize) + sy,
                        tftRusColor);
                } else {
                  if (tftRusBack >= 0)
                    tft.drawPixel(
...
// подсчет ширины строки с текущими настройками. возвращает int количество точек
// по горизонтали
//=======================================================================================
int tftRusTextWidth(String text) {
  //=======================================================================================
  return (int)(tftRusLength(text) * tftRusWidth * tftRusSize) +
         (trunc(tftRusStyle / 4) * tftRusCursiveLevel * tftRusSize);
}
...
int jamButton(uint8_t lay) {
  ...
  while (n <= lastBut && !flag) {
    if (but[n].layout == lay) {
      if (screenV) {
        if (x >= (but[n].xPosV) && x < (but[n].xPosV + But_Width) &&
            y >= (but[n].yPosV) && y < (but[n].yPosV + But_Height))
          flag = true;
      } else {
        if (x >= (but[n].xPosH) && x < (but[n].xPosH + But_Width) &&
            y >= (but[n].yPosH) && y < (but[n].yPosH + But_Height))
          flag = true;
      }
...
void drawBut(uint8_t id, uint8_t state, String alt) {
  ...
  if (state == B_BLOCK)
    ...
    if (type == B_BLUE)
      spr.pushImage(0, 0, But_Width, But_Height, (uint16_t *)But_);
    if (type == B_GREEN)
      spr.pushImage(0, 0, But_Width, But_Height, (uint16_t *)But_page);
    if (type == B_GOLD)
      spr.pushImage(0, 0, But_Width, But_Height, (uint16_t *)But_retro);
  } else if (type == B_GOLD)
    spr.pushImage(0, 0, But_Width, But_Height, (uint16_t *)But_fix);
